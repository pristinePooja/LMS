...
import { ScrollbarGeometry, ScrollbarPosition } from '@pristine/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[pristineScrollbar]',
    exportAs: 'pristineScrollbar'
})
export class PristineScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_pristineScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() pristineScrollbar: boolean = true;
    @Input() pristineScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'pristineScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.pristineScrollbar = coerceBooleanProperty(changes.pristineScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.pristineScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'pristineScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.pristineScrollbarOptions.currentValue);
